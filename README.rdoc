...
For do the task I decided using ElasticSearch, I know that for this data base I can do more easy and fast with ActiveRecord scopes
but this is not the point for a massive data base like Homestay with millions of rows (scalability) Its possible search by address with smalls
changes.
...
I changed the database because I prefer using Postgres than sqlite.

More things:

Create the Index Booking
- Booking.first.__elasticsearch__.index_document

Import Documents Elasticsearch Booking
-  Booking.__elasticsearch__.import

Do the same for the object Room.

I focus in the example with one guests because this database only focus in this problem.
to support others paradigms.

This is the code
- @bookings = (Booking.search filter:{or:[{range:{number_of_guests:{gte: params[:booking][:number_of_guests]}}},{range:{number_of_guests:{lt: params[:booking][:number_of_guests]}}}]},size:2001, sort:{room_id:{order: "asc"}}).records.to_a

I am tracking all the code with CodeClimate for know the quality.

For more questions please contact to me emacipbelmonte@gmail.com
